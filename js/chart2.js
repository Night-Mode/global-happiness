...
export async function renderChart2() {
  ...
  // Shared Y domain for consistency
  const allData = top10.concat(bottom10);
  const yDomainMax = d3.max(allData, d => d.total) * 1.1;
  ...
  function createLegend(sortKey) {
    ...
    const paddingTop = 30;
    const rowHeight = 24;
    const legendItemOffset = 50;

    const color = d3.scaleOrdinal().domain(keys).range(d3.schemeTableau10);
    const allKeys = ["total"].concat(keys);
    const legendHeight = paddingTop + legendItemOffset + allKeys.length * rowHeight;
    ...
    const groups = legend.selectAll("g")
      ...
      .enter()
      .append("g")
      .attr("transform", (d, i) => `translate(0, ${i * rowHeight + 30})`);
